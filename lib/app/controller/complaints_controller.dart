    ...
    if (response.statusCode == 200) {
      ...
      if (storeInfo != null) {
        ...
        storeInfo.forEach((data) {
          StoresModel dataInfo = StoresModel.fromJson(data);
          _stores.add(dataInfo);
        });
      }
      for (var item in details.orders!) {
        var storeName = stores.firstWhere((element) => element.uid == item.storeId).name;
  ...
  Future<void> onStoreModal() async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          scrollable: true,
          title: Text('Choose Store'.tr, style: const TextStyle(fontSize: 14, fontFamily: 'bold'), textAlign: TextAlign.center),
          content: Column(
            children: [
              SizedBox(
                height: 200.0, // Change as per your requirement
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return ListTile(
                        ...
                        leading: Radio(
                          fillColor: MaterialStateProperty.resolveWith(getColor),
                          value: stores[index].id.toString(),
                          groupValue: storeID,
                          onChanged: (e) {
                            storeID = e.toString();
                            var selected = stores.firstWhere((element) => element.id.toString() == storeID).name;
                            storeName = selected!;
                            update();
                            Navigator.pop(context);
                          },
  ...
  Future<void> onDriverModal() async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          scrollable: true,
          title: Text('Choose Driver'.tr, style: const TextStyle(fontSize: 14, fontFamily: 'bold'), textAlign: TextAlign.center),
          content: Column(
            children: [
              SizedBox(
                height: 200.0, // Change as per your requirement
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      ...
                      return ListTile(
                        textColor: ThemeProvider.appColor,
                        iconColor: ThemeProvider.appColor,
                        title: Text('${driversList[index].firstName} ${driversList[index].lastName}'),
                        leading: Radio(
                          fillColor: MaterialStateProperty.resolveWith(getColor),
                          ...
                          onChanged: (e) {
                            driverId = e.toString();
                            var selected = driversList.firstWhere((element) => element.id.toString() == driverId);
                            driverName = '${selected.firstName} ${selected.lastName}';
                            update();
                            Navigator.pop(context);
                          },
                        ),
