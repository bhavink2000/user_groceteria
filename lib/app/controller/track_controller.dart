...
import 'package:user/app/helper/pin_info.dart';
...
class TrackingController extends GetxController implements GetxService {
  ...
  PinInformation sourcePinInfo = PinInformation(
      locationName: "Start Location".tr, location: const LatLng(21.530435, 71.825037), pinPath: "assets/images/bike.png", avatarPath: "assets/images/bike.png", labelColor: Colors.blueAccent);
  PinInformation destinationPinInfo =
      PinInformation(locationName: "End Location".tr, location: const LatLng(21.536411, 71.834638), pinPath: "assets/images/dest.png", avatarPath: "assets/images/dest.png", labelColor: Colors.purple);
  ...
  void onInit() {
    ...
    if (googleMapsType == 'store') {
      ...
      destinationLocation = LocationData.fromMap({"latitude": storeLat, "longitude": storeLng});
      ...
      destinationLocation = LocationData.fromMap({"latitude": userLat, "longitude": userLng});
      var destPosition = LatLng(userLat, userLng);
      initialCameraPosition = CameraPosition(zoom: cameraZoom, tilt: cameraTilt, bearing: cameraBearing, target: destPosition);
      initialCameraPosition = CameraPosition(target: LatLng(driverLat, driverLng), zoom: cameraZoom, tilt: cameraTilt, bearing: cameraBearing);
      location = Location();
      polylinePoints = PolylinePoints();
      updatePinOnMap();
      setSourceAndDestinationIcons();
  ...
  Future<void> getDriverInfo() async {
    ...
    if (response.statusCode == 200) {
      ...
      currentLocation = LocationData.fromMap({"latitude": driverLat, "longitude": driverLng});
      updatePinOnMap();
      update();
    } else {
  ...
  Future<void> makePhoneCall() async {
    final Uri launchUri = Uri(scheme: 'tel', path: googleMapsType == 'store' ? storeMobile : driverMobile);
