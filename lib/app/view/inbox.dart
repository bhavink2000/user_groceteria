...
class _InboxScreenState extends State<InboxScreen> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(const Duration(seconds: 10), (Timer t) {
      Get.find<InboxController>().getChatList();
    });
  }
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: List.generate(
                      ...
                      (index) {
                        return value.chatList[index].fromId.toString() != value.uid.toString()
                            ? Container(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  Container(
                                                    ...
                                                    child: Text(value.chatList[index].message.toString(), style: const TextStyle(color: Colors.white, fontSize: 14)),
                                                  ),
                                                  value.yourMessage == true && (value.chatList.length - 1) == index
                                                      ? Container(
                                                          height: 15,
                                                          width: 15,
                                                          margin: const EdgeInsets.only(top: 8, left: 30, right: 30),
                                                          child: const CircularProgressIndicator(color: ThemeProvider.appColor),
                                                        )
                                                      : const SizedBox()
                                                ],
                                              ),
            ...
            child: Container(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  InkWell(onTap: () => value.sendMessage(), child: const Icon(Icons.send, color: ThemeProvider.appColor, size: 30)),
                ],
              ),
            ),
          ),
